...
-- 3. Adicionar coluna de status se não existir
DO $$ 
BEGIN 
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='cris_tech_ordens_servico' AND column_name='status') THEN
    ALTER TABLE cris_tech_ordens_servico ADD COLUMN status TEXT DEFAULT 'aberta' CHECK (status IN ('aberta', 'em_andamento', 'concluida', 'cancelada'));
  END IF;
END $$;

-- 4. Clientes (opcional, mas seguindo a lógica do usuário de 'qualquer usuário pode excluir')
